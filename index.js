...
export default class Cabinet {
  ...
  static #defaultLookups = {
    '.js': Cabinet.#jsLookup,
    '.jsx': Cabinet.#jsLookup,
    '.less': sassLookup, // Less and Sass imports are very similar
    '.sass': sassLookup,
    '.scss': sassLookup,
    '.styl': stylusLookup,
    '.svelte': Cabinet.#sfcLookup,
    '.ts': Cabinet.#tsLookup,
    '.tsx': Cabinet.#tsLookup,
    '.vue': Cabinet.#sfcLookup
  };

  constructor() {
    this.#lookups = { ...Cabinet.#defaultLookups };
  }
  ...
  lookup(options = {}) {
    ...
    let resolver = this.#lookups[extension];
    if (!resolver) {
   ...
   * @param  {Function} lookupStrategy - A resolver that accepts the options object used by `lookup`
   */
  register(extension, lookupStrategy) {
    this.#lookups[extension] = lookupStrategy;
  }
  ...
  static #jsLookup(options) {
    ...
    switch (type) {
      ...
      case 'es6': {
        ...
        return Cabinet.#commonJSLookup(options);
      }

      case 'webpack': {
        debug('using webpack resolver for es6');
        return Cabinet.#resolveWebpackPath({
          dependency,
          filename,
          directory,
          webpackConfig
  ...
  static #resolveFromTsAliasPath(resolvedTsAliasPath, extensions, fileSystem) {
  ...
  static #tsLookup({ dependency, filename, directory, webpackConfig, tsConfig, tsConfigPath, noTypeDefinitions, fileSystem }) {
    ...
    if (dependency && dependency.startsWith('#')) {
      ...
      const hashResult = Cabinet.#resolveHashImport(dependency, filename);
    ...
    const compilerOptions = Cabinet.#getCompilerOptionsFromTsConfig(tsConfig);
    const host = Cabinet.#getCompilerHost();
    ...
    if (!result && tsConfigPath && compilerOptions.baseUrl && compilerOptions.paths) {
      ...
      if (resolvedTsAliasPath) {
        result = Cabinet.#resolveFromTsAliasPath(resolvedTsAliasPath, extensions, fileSystem);
      }
    }
  ...
  static #commonJSLookup(options) {
    ...
    if (Cabinet.#isRelativePath(dependency)) {
      dependency = path.resolve(path.dirname(filename), dependency);
    }

    const tsCompilerOptions = Cabinet.#getCompilerOptionsFromTsConfig(tsConfig);
    ...
    if (allowMixedJsAndTs) {
      ...
      result = Cabinet.#tsLookup(options);
      if (result) {
  ...
  static #sfcLookup(options) {
    ...
    if (dependency.endsWith('.js') || dependency.endsWith('.jsx')) {
      return Cabinet.#jsLookup(options);
    }

    if (dependency.endsWith('.ts') || dependency.endsWith('.tsx')) {
      return Cabinet.#tsLookup(options);
    }
  ...
  static #webpackResolverByConfig = new Map();

  static #resolveWebpackPath({ dependency, filename, directory, webpackConfig }) {
    enhancedResolve ||= require('enhanced-resolve');

    webpackConfig = path.resolve(webpackConfig);
    let resolver = Cabinet.#webpackResolverByConfig.get(webpackConfig);
    let resolveConfig;
    ...
    try {
      if (!resolver) {
        ...
        Cabinet.#webpackResolverByConfig.set(webpackConfig, resolver);
      }

      // We don't care about what the loader resolves the dependency to.
      // We only want the path of the resolved file.
      dependency = Cabinet.#stripLoader(dependency);

      const lookupPath = Cabinet.#isRelativePath(dependency) ? path.dirname(filename) : directory;
   ...
   * @return {Boolean}
   */
  static #isRelativePath(filename) {
  ...
  static #getCompilerHost() {
    ts ||= require('typescript');
    Cabinet.#compilerHost ||= ts.createCompilerHost({});
    return Cabinet.#compilerHost;
  }

  static #getCompilerOptionsFromTsConfig(tsConfig) {
  ...
  static #resolveHashImport(dependency, filename) {
